...
foreach $body (@bodies) {
	...
	## create $filename-essay.settings.tex file
	$settings = "$filename-essay.settings.tex";
	...
	if ($pdfswitch == 1) {
	    `pdflatex $filename-essay 1>&2`;
	}
	else {
	    `latex $filename-essay 1>&2`;
	}
	unless ($quickswitch == 1) {
	    `bibtex $filename-essay 1>&2`;
	    if ($pdfswitch == 1) {
		`pdflatex $filename-essay 1>&2`;
		`pdflatex $filename-essay 1>&2`;
	    }
	...
	if ($pdfswitch == 0) {
	    `dvips -o $filename-essay.ps $filename-essay 1>&2`;
	    `epstopdf $filename-essay.ps 1>&2`;
	}

	## if open, bring to front
        `open $filename-essay.pdf`;
	## `open -a preview`;
...
##	$numtodos = `countcheckboxes $filename.abs.tex $filename.body.tex $filename.appendix.tex $filename.supplementary.tex 1>&2`;
	...
	$citenum = `grep \"Warning--I didn\'t find a database entry for\" $filename-essay.blg | wc -l`;
	$citenum =~ s/\s//g;
